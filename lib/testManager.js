...
class TestManager {
	...
	static async addAnswer(content, correct, message) {
		...
		const r = await testsCollection.updateOne({ "questions.messageId": message.reference.messageID }, { $addToSet: { "questions.$.answers": answer } });
		console.log(r);
		message.react("👌");
	}
	/**
	...
	static async isResponseToQuestion(message) {
		if (!message.guild?.available || !message.reference?.messageID) { return false; }
		const db = await dbManager.db();
		const testsCollection = db.collection(`${message.guild.id}Tests`);
...
module.exports = { TestManager, QuestionType };
