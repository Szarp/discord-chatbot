...
export async function createTestChannel(guild, channelName) {
    ...
    try {
        ...
        const channel = await createPrivateChannel2(guild, channelName, teacherRoleId, guild.roles.everyone);
        return channel;
    } catch (e) {
        console.log(e);
        return null
 ...
 * @param {string} denyId Id of user/group wgo can't see the chat
 */
export async function createPrivateChannel2(guild, channelName, allowId, denyId) {
    // TODO move channel to PRIVATE section -> need to capture sectionsId's
    try {
        const channel = await guild?.channels.create(channelName, {
            type: "text", //This create a text channel, you can make a voice one too, by changing "text" to "voice"
